...

{
	      ...
	      <link rel="stylesheet" type="text/css" media="all" href="<?php bloginfo('url');?>/?wpcss_action=show_css<?php
								...
								if (   (  current_user_can('edit_theme_options') or   function_exists('chpr_roller_is_allowed_to_all') )) echo "&amp;rnd=".rand(0,1024); ?>" data-apply-prefixfree />
...
function CSSHero_export_plugin_setup_menu(){
        add_menu_page( 'CSSHero_export Plugin Page', 'CSSHero Export/Import', 'manage_options', 'CSSHero_export-plugin', 'CSSHero_export_init' );
}
 
function CSSHero_export_init(){
		...
		<h2>Download active settings</h2>
		<p>This keeps your active settings, eg site personalizations, colors, etc. (without your editing history)</p>
		<form method="post">
			
			<input type="submit" name="csshero_export_current_settings_to_file" value="Download File">
		</form>
		<hr>
		<h2>Upload a current_csshero_settings.txt file and replace current state</h2>
		...
		<form method="post" enctype="multipart/form-data">
			...
			<p>
			<input type="submit" name="csshero_import_current_settings_button" value="Activate">
			</p>
		</form>
		<?php
}
